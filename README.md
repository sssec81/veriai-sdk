...
The repository is split into small Cargo crates:
...
└── verifier-service   # Axum service for independent receipt verification

examples/
└── 01-chat-demo       # OpenAI-compatible chat server and local proxy example
```
...
sequenceDiagram
    ...
    Note over Proxy: 3. Assemble CWT Claims (6000-6007, 6011-6012)
    Note over Proxy: 4. Sign Receipt with Ephemeral Ed25519 Key
    Proxy->>Client: ChatCompletionResponse + receipt (optional inline verification)
...
```
The response contains a completion, a signed receipt, and (in mock mode) an
inline verification summary. The default mock runtime uses a fixed test model
hash. When `VERIAI_RUNTIME=llama_cpp`, the server hashes the configured GGUF
file on startup. Attestation is still provided by the mock provider unless the
real-hardware feature is used:
```json
{
...
`EXPECTED_PCR0`. The demo hashes the canonical serialized request, the
configured model file at startup, and the exact completion returned by
`llama-cli`. In Nitro, the model and proxy image are also covered by PCR0.
