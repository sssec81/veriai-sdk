...
impl AttestationProvider for NitroAttestationProvider {
    async fn generate(
        ...
        let fd = nsm_init();
        if fd < 0 {
            return Err(AttestationError::HardwareUnavailable("Failed to open /dev/nsm driver".to_string()));
        }
        ...
        match response {
            ...
            Response::Error(err) => Err(AttestationError::ValidationError(format!("NSM process request failed: {:?}", err))),
            _ => Err(AttestationError::ValidationError("Unexpected response type from NSM".to_string())),
        }
    }
