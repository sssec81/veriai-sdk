...
async fn test_adversarial_valid_receipt() {
    ...
    let receipt = generator.generate_receipt(model_hash, input_hash, output_hash, client_nonce).await.unwrap();
    let res = verifier.verify(&receipt, model_hash, input_hash, output_hash, client_nonce, &pcr0).await.unwrap();
...
async fn test_adversarial_tampered_output() {
    ...
    let res = verifier.verify(&receipt, model_hash, input_hash, tampered_output, client_nonce, &pcr0).await.unwrap();
...
async fn test_adversarial_wrong_model_hash() {
    ...
    let res = verifier.verify(&receipt, wrong_model, input_hash, output_hash, client_nonce, &pcr0).await.unwrap();
...
async fn test_adversarial_invalid_signature() {
    ...
    let mut receipt = generator.generate_receipt(model_hash, input_hash, output_hash, client_nonce).await.unwrap();
...
async fn test_adversarial_expired_receipt() {
    ...
    let res = verifier.verify(&tampered_receipt, model_hash, input_hash, output_hash, client_nonce, &pcr0).await.unwrap();
...
async fn test_adversarial_replay_attack() {
    ...
    let res1 = verifier.verify(&receipt, model_hash, input_hash, output_hash, client_nonce, &pcr0).await.unwrap();
    assert!(res1.valid);

    // Second verification with identical receipt (replay attack, expected failure)
    let res2 = verifier.verify(&receipt, model_hash, input_hash, output_hash, client_nonce, &pcr0).await.unwrap();
    assert!(!res2.valid);
    assert_eq!(res2.error, Some("Sequence number is out of order (not monotonic)".to_string()));
}
