...
async fn test_verifier_end_to_end() {
    ...
    let verify_again_res = verifier
        ...
        .await
        .expect("Should return Ok VerificationResult");

    assert!(!verify_again_res.valid);
    assert_eq!(
        verify_again_res.error,
...
async fn test_verifier_payload_mismatch_errors() {
    ...
    assert_eq!(
        res.error,
        Some("Input hash does not match expected value".to_string())
    );
    ...
    assert_eq!(
        res.error,
        Some("Client nonce does not match Nitro document nonce".to_string())
    );
    ...
    assert_eq!(
        res.error,
        Some("PCR0 value does not match expected value".to_string())
    );
}
