use crate::nsm::schema::AttestationDoc;
...
/// Generates a mock signed COSE_Sign1 attestation document
pub fn get_attestation_document(
    ...
    let payload = doc.to_binary().map_err(|e| e.to_string())?;
    ...
    // Convert signature to SEC1 DER or raw bytes as required by COSE.
    // In COSE, ES384 signature is represented as raw R || S bytes (96 bytes).
    let sig_bytes = signature.to_bytes().to_vec();
    cose_sign1.signature = sig_bytes;

    cose_sign1.to_vec().map_err(|e| format!("Failed to serialize COSE_Sign1: {}", e))
}
